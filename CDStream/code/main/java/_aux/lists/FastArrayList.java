...
public class FastArrayList<T> implements Iterable<T> {
    ...
    @NonNull @Getter private final int capacity;
...
//    Specific methods
    ...
    public void add(T item){
        if (list.size() == capacity){
            throw new RuntimeException(String.format("FastArrayList is full; requested size = %d; capacity = %d" , list.size()+1, capacity));
        }
        list.add(item);
    }
    ...
    public void addAll(FastArrayList<T> items){
        if (list.size() == capacity){
            throw new RuntimeException(String.format("FastArrayList is full; requested size = %d; capacity = %d" , list.size()+items.size(), capacity));
        }
