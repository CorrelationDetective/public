...
public class CorrelationBounding {
    ...
    public double getCCGeometricMean(List<Cluster> LHS, List<Cluster> RHS){
        double out = 1;
        List<Cluster> allClusters = new ArrayList<>(LHS); allClusters.addAll(RHS);
        for (Cluster c: allClusters){
            out *= c.listOfContents.size();
        }
        return Math.pow(out, 1.0/allClusters.size());
    }

    public CorrBoundTuple calcBound(List<Cluster> LHS, List<Cluster> RHS, double shrinkFactor, boolean setState) { // it returns an array of lower, higher,
        ...
        if (setState){
            boundTuple.setState(HB.tau, LHS.size() + RHS.size() == 2 ? 0: HB.minJump, getCCGeometricMean(LHS, RHS),
                    shrinkFactor, HB.maxApproximationSize);
        }
