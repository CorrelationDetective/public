...
//    public static int distType=2; // 1 = l1, 2=l2 // warning: in the current code this does not change anythin
    ...
    public Cluster(int clusterId, TimeSeries centroid, int creationTime, int n) {
    ...
    public void addSubCluster(Cluster sc){
        if (this.subClusters == null){
            this.subClusters = new ArrayList<>();
        }
        this.subClusters.add(sc);
    }
    ...
    public Cluster splitOff(Cluster other, TimeSeries[] timeSeries, int t){
        if (this.listOfContents.equals(other.listOfContents)){
            return this;
        }

        List<Integer> newContent = (List<Integer>) this.listOfContents.clone();
        newContent.removeAll(other.listOfContents);
        Cluster newCluster = new Cluster(this.clusterId, timeSeries[newContent.get(0)], t, timeSeries.length);
        newCluster.computeDiameter(timeSeries, timeSeries[0].w);

//        Set subclusters
        for (Cluster sc: this.subClusters){
//            If there's overlap, also split off
            if (sc.listOfContents.stream().anyMatch(i -> other.listOfContents.contains(i))){
                sc = sc.splitOff(other, timeSeries, t);
                sc.parent = newCluster;
            }
            try{
                newCluster.addSubCluster(sc);
            } catch (NullPointerException e){
                System.out.println("Nullpointer for subcluster, subcluster is " + sc);
            }
        }
