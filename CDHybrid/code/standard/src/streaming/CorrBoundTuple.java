...
//        Handle between bounds
    ...
    public double setState(double corrThreshold, double minJump) {
        double newThreshold = corrThreshold;

        if (minJump >= 0){ // minJump and irreducibility
            double jumpBasedThreshold = this.max2CorrLowerBound + minJump;
            newThreshold = Math.max(jumpBasedThreshold, corrThreshold);
        }

        if ((lower < newThreshold) && (upper >= newThreshold)){
            this.state = 0;
        } else if (upper < newThreshold){
            this.state = -1;

        } else if (lower >= newThreshold){
            this.state = 1;
